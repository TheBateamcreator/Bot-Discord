...
module.exports = {
    ...
    options: [
        {
            type: 'channel',
            name: 'salon',
            description: 'Salon à lock',
            required: true,
            autocomplete: false
        }, {
            type: 'role',
            name: 'role',
            description: 'Rôle à lock',
            required: false,
            autocomplete: false
        }
    ],
    ...
    async run(bot, message, args) {
        ...
        try {
            ...
            .setTitle("Lock")
            .setDescription(`
            **${message.user.tag}** a utilisé la commande \`/lock\` :
            **Salon lock** : ${channel}
            **Rôle lock** : ${role}`)
            .setTimestamp()
            .setColor(bot.color)
            .setThumbnail(bot.user.displayAvatarURL({dynamic: true}))
            .setFooter({text: `Lock par : ${message.user.tag}`, iconURL: bot.user.displayAvatarURL({dynamic: true})})
